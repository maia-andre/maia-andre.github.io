...
describe('conteúdo real — vinte e quatro artigos publicados', () => {
  ...
  it('a listagem mostra os 24 publicados na ordem da RN-02 (data desc; empate → alfabético)', () => {
    const titulos = parsePage('artigos/index.html')
      .querySelectorAll('main .item-titulo a')
      .map((a) => a.text.trim());
    expect(titulos).toHaveLength(24);
    // A Rainha Vermelha (2026-07-28, sem empate) abre a lista; empate duplo
    ...
    expect(titulos[0]).toMatch(/^A Rainha Vermelha/);
    expect(titulos[1]).toMatch(/^O moletom/);
    expect(titulos[2]).toMatch(/^Rápido demais/);
    expect(titulos[3]).toMatch(/^O empréstimo/);
    expect(titulos[4]).toMatch(/^O réu/);
    expect(titulos[5]).toMatch(/^Do outro lado/);
    expect(titulos[6]).toMatch(/^O dia em que descobri/);
    expect(titulos[7]).toMatch(/^Para sair/);
    expect(titulos[8]).toMatch(/^Quando o requisito/);
    expect(titulos[9]).toMatch(/^Organizar/);
    expect(titulos[10]).toMatch(/^Todo software/);
    expect(titulos[11]).toMatch(/^A névoa/);
    expect(titulos[12]).toMatch(/^Os canteiros/);
    expect(titulos[13]).toMatch(/^Carta/);
    expect(titulos[14]).toMatch(/^A pergunta/);
    expect(titulos[15]).toMatch(/^A resposta/);
    expect(titulos[16]).toMatch(/^A memória/);
    expect(titulos[17]).toMatch(/^O menino/);
    expect(titulos[18]).toMatch(/^A cidade/);
    expect(titulos[19]).toMatch(/^O velório/);
    expect(titulos[20]).toMatch(/^O jardineiro/);
    expect(titulos[21]).toMatch(/^O silêncio/);
    expect(titulos[22]).toMatch(/^A régua/);
    expect(titulos[23]).toMatch(/^Construindo/);
  });
});
